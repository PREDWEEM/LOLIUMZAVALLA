...
"""Configuración y calibraciones de la plataforma PREDWEEM multisitio."""
...
def _active_site_slug() -> str:
    ...
    if streamlit_module is not None:
        try:
            return str(
                streamlit_module.session_state.get(
                    "selected_lolium_site", "zavalla"
                )
            ).strip().lower()
        except Exception:
            pass
...
class MultisiteConfig:
    """Configuración base reemplazada dinámicamente por la localidad activa."""
